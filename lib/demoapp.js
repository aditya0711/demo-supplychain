...
function compileSearch() {
  return function(scope){
    ...
    const compileList = [{
      searchable: ['User', 'StoreItem','Product'],
...
function createProductArgs(product) {
    // validate input
    if (product === undefined) throw new Error('Product: product undefined');
    if (product.id === undefined) throw new Error('Product: id undefined');
    if (product.name === undefined) throw new Error('Product: name undefined');
    if (product.description === undefined) throw new Error('Product: desc undefined');
    if (product.manufacturingDate === undefined) throw new Error('Product: manufacturing date undefined');
    if (product.manufacturingLocation === undefined) throw new Error('Product: manufacturing location  undefined');
    if (product.parentID === undefined) throw new Error('Product:parent ID   undefined');
    ...
    const args = {
        ...
        _parentId :product.parentID
    };
    return args;
}

function createComponentArgs(component) {
    // validate input
    if (component === undefined) throw new Error('Component: product undefined');
    if (component.id === undefined) throw new Error('Component: id undefined');
    if (component.name === undefined) throw new Error('Component: name undefined');
    if (component.description === undefined) throw new Error('Component: desc undefined');
    if (component.manufacturingDate === undefined) throw new Error('Component: manufacturing date undefined');
    if (component.manufacturingLocation === undefined) throw new Error('Component: manufacturing location  undefined');
    if (component.parentID === undefined) throw new Error('Component:parent ID   undefined');
    // create arg
    const args = {
        _id: component.id,
        _name: component.name,
        _description: component.description,
        _manufacturingDate: component.manufacturingDate,
        _manufacturingLocation: component.manufacturingLocation,
        _pid : component.parentID,
...
function addProduct(adminName, product){
   //define the contract
    var contract_address ;
    console.log("Add Product demoapp.js: " + JSON.stringify(product))
    return function(scope) {
       const contractName = AI.subContractsNames.ProductManager;
       ...
       return rest.callMethod(adminName, contractName, methodName, args)(scope)
           .then(function (scope) {
               ...
               var result = scope.contracts[contractName].calls[methodName];
               console.log("Scope : " + JSON.stringify(result));
               return result;
           });
       return scope;
   }
...
function addComponent(adminName, component){
   //define the contract
    var contract_address ;
    console.log("Add Component demoapp.js: " + JSON.stringify(component))
   return function(scope) {
       const contractName = AI.subContractsNames.ProductManager;
       const methodName = "addComponent";
       const args = {
           _id                      : component.id,
           _name                    : component.name,
           _description             : component.description,
...
function getAllProductsQuery(adminName){
    return function(scope) {
        rest.verbose('getProductQuery');

        return nop(scope)
            .then(rest.query('Product'))
            .then(function(scope) {
                const result = scope.query.slice(-1)[0];
                console.log('query', result);
...
function getProductAddress(adminName, product_id){
    return function(scope){
        rest.verbose('getProduct');
        const contractName = AI.subContractsNames.ProductManager;
        const methodName = 'getAddressById';
        const args = { _id: product_id };
        ...
        return rest.callMethod(adminName, contractName, methodName, args)(scope)
            .then(function(scope){
                ...
                if (scope.product[product_id] === undefined)
                    scope.product[product_id] = {};

                scope.product[product_id].address = result;
                return scope;
            })
    }
}

function getProductState(adminName, product_id){
    rest.verbose('getProductState', {product_id});
    const contractName = 'Product';
    return function(scope){
        return getProductAddress(adminName, product_id)(scope)
            .then(function(scope){
                const address = scope.product[product_id].address;
                return rest.getState(contractName, address)(scope);
            })
            .then(function(scope){
                 scope.product[product_id].name                     = scope.states[contractName].name;
                 scope.product[product_id].description              = scope.states[contractName].description;
                 scope.product[product_id].manufacturingLocation    = scope.states[contractName].manufacturingLocation;
                 scope.product[product_id].manufacturingDate        = scope.states[contractName].manufacturingDate;
                 scope.product[product_id].componentList            = scope.states[contractName].componentList;
                 scope.product[product_id].productType              = scope.states[contractName].productType;
                 scope.product[product_id].parentID                 =scope.states[contractName].parentID;
                return scope;
         })
    }
}
...
module.exports = function(contractsPath) {
  ...
  return {
    ...
    createComponentArgs: createComponentArgs,
    compileSearch: compileSearch,
    getAdminInterface: getAdminInterface,
    getEnums: getEnums,
    getUser: getUser,
    getAllProducts: getAllProductsQuery,
      getProductAddress: getProductAddress,
