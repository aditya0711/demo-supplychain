...
function createProduct(adminName, id, name, price){
    return function(scope) {
        if (id      === undefined) throw new Error('Product: INPUT id undefined');
        if (name    === undefined) throw new Error('Product: INPUT name undefined');
        if (price   === undefined || price === 0) throw new Error('Product: INPUT price undefined');
            ...
            return productMgrLib.createProduct(adminName, id, name, price)(scope)
                .then(function (scope) {
                    ...
                    if (result == E.SUCCESS) {
                        ...
                        throw new Error("Product exists: " + id);
                    } else {
                        throw new MethodError("Error", 'createProduct', result);
                    }
                    return scope;
                })
        }
            ...
            return productMgrLib.link(adminName, parentId, childId, quantity)(scope)
                .then(function (scope) {
                    ...
                    if (result == E.SUCCESS) {
                        ...
                        throw new Error("Adding recursive child: " + childId);
                    } else if (result == E.NOT_FOUND) {
                        throw new Error("Product not found: " + childId);
                    }
                    else if (result == E.EXISTS) {
                        throw new Error("Sub Product Exists");
                    }else {
                        throw new MethodError("Error", 'link', result);
                    }
                    return scope;
                });
        }
