...
const productMgrLib = require('./productManager');
...
function createProductArgs(id, name, price) {
    // validate input
    if (id      === undefined) throw new Error('Product: INPUT id undefined');
    if (name    === undefined) throw new Error('Product: INPUT name undefined');
    if (price   === undefined) throw new Error('Product:  price undefined');
    INPUT
    // create args
    const args = {
        id      : id,
...
function createProduct(adminName, id, name, price){
    ...
    return function(scope) {
        ...
        if (id      === undefined) throw new Error('Product: INPUT id undefined');
        if (name    === undefined) throw new Error('Product: INPUT name undefined');
        if (price   === undefined) throw new Error('Product: INPUT price undefined');
        else {
            const contractName = AI.subContractsNames.ProductManager;
            rest.verbose('createProduct', id, name, price);

            return productMgrLib.createProduct(adminName, id, name, price)(scope)
                .then(function (scope) {
                    scope.products = [];
                    //returns Error code enum
                    const result = scope.contracts[contractName].calls['createProduct'];
                    const E = getEnums().ErrorCodesEnum;
                    if (result == E.SUCCESS) {
                        if (scope.products[id] === undefined)
                            scope.products[id] = {};
                    } else if (result == E.EXISTS) {
                        throw new Error("Product exists: " + id); // TODO add a string conversion
                    } else {
                        throw new MethodError(contractName, 'createProduct', result);
                    }
                    return scope;
                })
        }
...
function link(adminName, parentId, childId, quantity){
    ...
    return function(scope){
        if (parentId   === undefined) throw new Error('Product: INPUT parentId undefined');
        if (childId    === undefined) throw new Error('Product: INPUT childId undefined');
        if (quantity   === undefined || quantity === 0) throw new Error('Product: INPUT quantity undefined');
        else {
            ...
            return productMgrLib.link(adminName, parentId, childId, quantity)(scope)
                .then(function (scope) {
                    const result = scope.contracts[contractName].calls[method];
                    if (result == E.SUCCESS) {
                        //FIXME what happens here?
                        // if(scope.products[parentId].link === undefined)
                        //     scope.products[parentId].link = [];
                        //
                        // scope.products[parentId].push(childId);
                        //console.log("PRINT the LINKS INSIDE A PRODUCT: " + scope.products[parentId])
                        return scope;
                    }
                    else if (result == E.RECURSIVE) {
                        throw new Error("Adding recursive child: " + childId); // TODO add a string conversion
                    } else if (result == E.NOT_FOUND) {
                        throw new Error("Product not found: " + childId); // TODO add a string conversion
                    }
                    else if (result == E.EXISTS) {
                        throw new Error("Sub Product Exists"); // TODO add a string conversion
                    }else {
                        throw new MethodError(contractName, 'link', result);
                    }
                    return scope;
                });
        }
...
function getProductsQuery(adminName, searchTerm) {  //search term = productID
    return function(scope) {

        if(searchTerm === undefined){
            rest.verbose('getProducts');
            return nop(scope)
        ...
        return nop(scope)
            .then(rest.query('Product?_id=eq.' + searchTerm))
            .then(function(scope) {
                const result = scope.query.slice(-1)[0];
                //console.log('query', result);
                return scope;
