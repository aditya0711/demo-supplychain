...
describe('Govern User Manager tests', function() {
  ...
  it('Change state from PENDING to ACTIVE', function(done) {
    const username = util.uid('GovernUser');
    const pwHash = util.toBytes32('1234'); // FIXME this is not a hash
  ...
  it.only('As ADMIN, Read all pending MEMBERS, and APPROVE a few of them', function(done) {
    const username = util.uid('GovernUser');
    const count = 1;
    ...
    rest.setScope(scope)
      .then(userManager.uploadContract(adminName, adminPassword))
      // create all users
      .then(factory_createGovernUsers(adminName, username, count))
      // query for all the users
      .then(userManager_getUserByRoleState(GovernUserRole[GovernUserRole.MEMBER], GovernUserState[GovernUserState.PENDING]))
      .then(function(scope) {
        const usersToBeApproved = scope.result.slice(0,3);
        return Promise.each(usersToBeApproved, function(user) { // for each user
          return rest.setScope(scope)
            .then(userManager.handleEvent(adminName, user.username, GovernUserEvent.APPROVE)) // approve user
            .then(function(scope) {
              const result = scope.result;
              console.log(result);
              assert.equal(result.errorCode, ErrorCodes.SUCCESS, 'handleEvent should return ErrorCodes.SUCCESS');
...
function factory_createGovernUsers(adminName, username, count) {
  const pwHash = util.toBytes32('1234'); // FIXME this is not a hash
...
function userManager_getUserByRoleState(role, state) {
  return function (scope) {
    return rest.setScope(scope)
      // query for all the users
      .then(rest.query(`GovernUser?state=eq.${state}`))
      .then(function(scope) {
        const unfiltered = scope.query.slice(-1)[0];
        const filtered = unfiltered.filter(function(governUser) {
          return (governUser.role === role);
        });
