...
const GovernUserRole = governuser.GovernUserRole;
const GovernUserState = governuser.GovernUserState;
...
describe('Govern User tests', function() {
  ...
  it('Create Contract', function(done) {
    ...
    rest.setScope(scope)
      ...
      .then(governuser.waitQuery(`username=eq.${username}`, 1))
      .then(function(scope) {
  ...
  it('Construct GovernUser', function(done) {
    ...
    rest.setScope(scope)
      .then(governuser.uploadContract(adminName, adminPassword, args))
      .then(governuser.waitQuery(`username=eq.${username}`, 1)) // must be here
      .then(governuser.get(username))
      .then(function(scope) {
        // only 1 result
        assert.equal(scope.result.length, 1, 'one and only one');
        // check values
  ...
  it('Set role / state', function(done) {
    const username = util.uid('GovernUser');
    const pwHash = util.toBytes32('1234'); // FIXME this is not a hash
    const role = GovernUserRole.MEMBER;
    const state = GovernUserState.PENDING;
    ...
    rest.setScope(scope)
      ...
      .then(function(scope) {
        ...
        assert.equal(result.state, GovernUserState[state], 'state'); // from constructor
        return scope;
      })
