...
describe('ProductManager tests', function () {
  ...
  it('get product', function (done) {
    ...
    rest.setScope(scope)
      ...
      .then(productManager.getProduct(util.toBytes32(parent.id)))
      .then(function (scope) {
  ...
  it('add multi level', function (done) {
    ...
    // add all components
      ...
      // get the product tree
      .then(productManager.getProduct(util.toBytes32(compArray[0].id)))
      .then(function (scope) {
        const product = scope.result;
        assert.equal(product['_id'], compArray[0].id, 'tree');
        assert.equal(product['children'][0]['_id'], compArray[1].id, 'tree');
        assert.equal(product['children'][0]['children'][0]['_id'], compArray[2].id, 'tree');
        return scope;
      })
      .then(productManager.getProduct(util.toBytes32(compArray[3].id)))
      .then(function (scope) {
        const product = scope.result;
        assert.equal(product['_id'], compArray[3].id, 'tree');
        assert.equal(product['children'][0]['_id'], compArray[4].id, 'tree');
        assert.equal(product['children'][0]['children'][0]['_id'], compArray[2].id, 'tree');
        return scope;
      })
      .then(function (scope) {
