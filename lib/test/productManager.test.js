...
describe('ProductManager tests', function () {
  ...
  it('add multi level', function (done) {
    ...
    // add all components
      ...
      .then(productManager.getProductTree(compArray[3].id))
      .then(function (scope) {
        const tree = scope.result;
        assert.equal(tree['_id'], compArray[3].id, 'tree');
        assert.equal(tree['children'][0]['_id'], compArray[4].id, 'tree');
        assert.equal(tree['children'][0]['children'][0]['_id'], compArray[2].id, 'tree');
        return scope;
      })
      .then(function (scope) {
...
function getProductTest(id) {
  return function (scope) {
    ...
    return rest.setScope(scope)
      .then(productManager.getProductTree(id))
