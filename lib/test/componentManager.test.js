...
describe('ComponentManager tests', function() {
  ...
  it('add sub component', function(done) {
    ...
    rest.setScope(scope)
      ...
      .then(rest.query(`Component?_name=eq.${parent.name}`))
      .then(function(scope) {
      ...
      .then(function(scope) {
        return Promise.each(compArray, function(comp) { // for each component
          return rest.setScope(scope)
            .then(createComponentTest(adminName, comp.id, comp.name, ErrorCodesEnum.SUCCESS))
            .then(rest.waitQuery(`Component?_name=eq.${comp.name}`, 1))
  ...
  it('add multi level', function(done) {
    ...
    // create all component names
    var count = 100;
    const compArray = [...Array(8).keys()].map(function(item, index) {
      return {
        id: parent.id + '_' + index,
        name: parent.name + '_' + index,
      };
...
for( var i = 0 ; i < 1000; i++) {
  it.only('create batch components', function(done) {
    ...
    const count = 200;
    console.log('loop counter: ', ++loopCounter, 'count', i*count);
