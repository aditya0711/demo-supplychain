...
describe('ComponentManager tests', function () {
  ...
  before(function (done) {
    rest.setScope(scope)
      ...
      .then(componentManager.uploadContract(adminName, adminPassword))
      .then(function (scope) {
        done();
      }).catch(done);
  });
  ...
  it('add sub component', function (done) {
    ...
    rest.setScope(scope)
      ...
      .then(rest.waitQuery(`Component?_id=eq.${parent.id}`, 1))
      .then(function (scope) {
      ...
      .then(function (scope) {
        return Promise.each(compArray, function (comp) { // for each component
          return rest.setScope(scope)
            ...
            .then(rest.waitQuery(`Component?_id=eq.${comp.id}`, 1))
        }).then(function () { // all done
          return scope;
        });
      })
      .then(rest.waitQuery(`Component?_id=like.%${parent.id}%`, count))
