  ...
  return function (scope) {
    ...
    return rest.setScope(scope)
      ...
      .then(function (scope) {
        ...
        const componentCsv = toCsv(components.slice(1)); // skip the first 0000 entry
        const query = `Product?address=in.${componentCsv}`;
        return rest.query(query)(scope);
      })
      .then(function (scope) {
...
function toCsv(array) {
  return array.reduce(function(prevVal, elem) {
    // due to a bug in CIRRUS, leading zeros must be trimmed
    while (elem.slice(0,1) == '0') {
      elem = elem.slice(1);
    }
