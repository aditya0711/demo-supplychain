...
function getProducts() {
  return function (scope) {
    ...
    return rest.setScope(scope)
      .then(getState())
      .then(function (scope) {
        const components = scope.result.components.slice(1); // skip the first 0000 entry
        const trimmed = trimArray(components); // trim leading zeros due to bug in cirrus
        const componentCsv = util.toCsv(trimmed); // generate csv string
        return rest.query(`Product?address=in.${componentCsv}`)(scope);
      })
      .then(function (scope) {
