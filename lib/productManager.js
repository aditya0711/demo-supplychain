...
function getProductTree(id) {
  return function (scope) {
    rest.verbose('getProductTree', id);
    const id32 = util.toBytes32(id);

    return rest.setScope(scope)
      .then(get(id32))
      .then(function (scope) {
        console.log('getProductTree: tree: ', JSON.stringify(scope.result, null, 2));
        return scope;
      });
  }
}
...
function get(id32) {
  return function (scope) {
    rest.verbose('get', id32);

    return rest.setScope(scope)
      .then(rest.query(`Product?_id32=eq.${id32}`))
      .then(function (scope) {
        const result = scope.query.slice(-1)[0];
        if (result.length == 0) throw new Error(`Not Found: id32: ${id32}`);
        if (result.length > 1) throw new Error(`Non Unique: id32: ${id32}`);
        ...
        // scan all children
        return Promise.each(product.children, function (child) {
          const childScope = {};
          return rest.setScope(childScope)
            .then(get(child['id32']))
            .then(function (childScope) {
              child.address = childScope.result.address;
              child._id = childScope.result._id;
              child._name = childScope.result._name;
              child._price = childScope.result._price;
              child.children = childScope.result['children'];
              return childScope;
            });
        }).then(function () {
